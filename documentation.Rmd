...
From the above results, a general recommendation is that Green OA cannot be considered a 'black box' in the context of understanding impact metrics, as it includes multiple archiving routes, and interacts strongly with other forms of OA.

An alternative may therefore be to create separate categories for Green OA including and excluding PMC articles, or for Green OA including and excluding journal-based OA. 
...
abbreviate_classifications <- function(d) {
  d %>%
    mutate(classification = factor(LR_main_field,
...
# Coverage per classification
# Overall coverage
p1 <- read_csv("data/oa_altmetrics_coverage_classification.csv") %>%
  abbreviate_classifications() %>%
  pivot_longer(n_blog:n_wikipedia,
               names_to = "indicator",
               values_to = "items_covered") %>%
  ...
  summarize(coverage = sum(items_covered) / sum(n_items)) %>%
  ggplot(aes(x = classification, y = coverage)) +
  geom_bar(stat = "identity") +
  facet_wrap(. ~ indicator, nrow = 1) +
  scale_y_continuous(labels = scales::percent) +
  labs(x = "",
...
# Relative coverage per group
p2 <- read_csv("data/oa_altmetrics_coverage_classification.csv") %>%
  abbreviate_classifications() %>%
  pivot_longer(n_blog:n_wikipedia,
               names_to = "indicator",
               values_to = "items_covered") %>%
  mutate(indicator = factor(indicator,
         ...
         type = factor(type, 
                       ...
                       labels = c("OA", "Non-OA"))) %>%
  mutate(coverage = items_covered / n_items) %>%
  group_by(classification, indicator) %>%
  mutate(rel_coverage = coverage / (sum(items_covered) / sum(n_items))) %>%
  ggplot(aes(x = classification, y = rel_coverage)) +
  ...
  labs(x = "",
       y = "Relative Coverage",
       title = "Relative Coverage: All OA vs Non-OA",
       fill = "")
