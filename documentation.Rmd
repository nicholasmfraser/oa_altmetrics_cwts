...
Coverage is defined as the number of articles that are mentioned by a particular altmetric sources The *relative coverage* per group is
...
There are 11,149 distinct journal titles associated with our WOS sample. In the following section, journals are grouped into percentiles on the basis of their Journal Impact Factor.
...
read_csv("data/altmetrics_coverage_journal.csv") %>%
  filter(items >= 100) %>%
  inner_join(journal_ifs, by = "journal") %>%
  pivot_longer(blogs:wikipedia,
               names_to = "indicator",
               values_to = "items_covered") %>%
  group_by(if_bin, indicator) %>%
  mutate(coverage = sum(items_covered) / sum(items)) %>%
  ggplot(aes(x = if_bin, y = coverage)) +
  geom_bar(stat = "identity", position = "dodge") +
...
read_csv("data/altmetrics_coverage_journal_oa.csv") %>%
  filter(items >= 100) %>%
  inner_join(journal_ifs, by = "journal") %>%
  pivot_longer(blogs:wikipedia,
               names_to = "indicator",
               values_to = "items_covered") %>%
  mutate(type = factor(type, levels = c("open", "closed"))) %>%
  group_by(if_bin, indicator, type) %>%
  summarize(coverage = sum(items_covered) / sum(items),
  ...
  mutate(rel_coverage = coverage / (sum(items_covered) / sum(items))) %>%
  ggplot() +
  geom_point(aes(x = if_bin, y = rel_coverage, fill = type), 
             shape = 21, color = "grey10", size = 3, alpha = 0.75) +
  geom_hline(yintercept = 1, linetype = "dotted") +
  facet_wrap(~ indicator, nrow = 1) +
  scale_y_continuous(limits = c(0,2)) +
