...
- **Relative Altmetric Coverage**: The number of articles that are mentioned in a given altmetric source **for a specific subset of a population**, relative to the entire population. This can be explained with the following example: a population of 100 articles consists of 40 OA and 60 non-OA articles. The Twitter coverage of the entire population is 30%, but the coverage of the OA group is 50%. Thus, the relative coverage of the OA group is 50/30 = 1.66. An interpretation is that OA articles are 66% more likely to receive OA coverage than the population as a whole.
...
Higher altmetric coverage is observed in BS, LES and SSH than MCS and PSE. SSH has the highest coverage on blogs, whilst on Facebook and Twitter the highest coverage is observed in BHS. MCS has the lowest coverage across all indicators. 
...
Interestingly, whilst PSE and MCS were shown in the previous figure to have the lowest coverage of all subject classifications, the *relative* coverage of OA publications in PSE and MCS is generally higher than in BHS, LES and SSH. An exception to this is in policy mentions, where relative coverage of OA publication in PSE is similar to LES and SSH, whilst in MCS the relative coverage of OA publications is only slightly greater than 1. 
...
For the following analysis, the top 50 countries by publishing volume are included.
...
p1 <- read_csv("data/altmetrics_coverage_country.csv") %>%
  ...
  ggplot() +
  geom_bar(aes(x = reorder(country, desc(country)), y = items), stat = "identity") +
...
p2 <- read_csv("data/altmetrics_coverage_country.csv") %>%
  ...
  mutate(coverage = items_covered / items,
         ...
         country = countrycode::countrycode(country,
                                            ...
                                            destination = "country.name")) %>%
  ggplot() +
  geom_point(aes(x = reorder(country, desc(country)), y = coverage), 
             shape = 21, fill = "grey50", color = "grey10", size = 3, alpha = 0.5) +
...
egg::ggarrange(p1 + theme(plot.margin = margin(0, 30, 0, 0)),
               p2, 
               nrow = 1, 
               widths = c(1,6))
...
```{r echo = F, message = F, warning = F, fig.height = 12, fig.width = 16}
...
p1 <- read_csv("data/altmetrics_coverage_country_oa.csv") %>%
  ...
  mutate(type = factor(type, levels = c("open", "closed")),
         ...
         country = countrycode::countrycode(country,
                                            ...
                                            destination = "country.name")) %>%
  ggplot() +
  geom_bar(aes(x = reorder(country, desc(country)), y = items, fill = type), 
           stat = "identity") +
  facet_grid(region ~ ., scales = "free_y", space = "free") +
  expand_limits(y = 0) +
  scale_y_continuous(labels = scales::comma) +
...
p2 <- read_csv("data/altmetrics_coverage_country_oa.csv") %>%
  ...
  mutate(type = factor(type, levels = c("open", "closed")),
         ...
         country = countrycode::countrycode(country,
                                            ...
                                            destination = "country.name")) %>%
  ggplot() +
  geom_point(aes(x = reorder(country, desc(country)), y = coverage, fill = type), 
             shape = 21, color = "grey10", size = 3, alpha = 0.5) +
  facet_grid(region ~ indicator, scales = "free_y", space = "free") +
  expand_limits(y = 0) +
  scale_y_continuous(labels = scales::percent) +
...
p3 <- read_csv("data/altmetrics_coverage_country_oa.csv") %>%
  ...
  mutate(rel_coverage = coverage / (sum(items_covered) / sum(items))) %>%
  ggplot() +
  geom_point(aes(x = reorder(country, desc(country)), y = rel_coverage, fill = type), 
             shape = 21, color = "grey10", size = 3, alpha = 0.5) +
  geom_hline(yintercept = 1, linetype = "dotted") +
  facet_grid(region ~ indicator, scales = "free_y", space = "free") +
  expand_limits(y = 0) +
...
Here we look at some of the most interesting results from the individual units in detail, and explore interactions between different units:
...
Given that policy documents focus heavily on Western, English-language countries, why is the coverage of Gold OA publications so low? 
...
Can we correct the coverage-OA types graph, only including papers authored from US, GB, AU, CA?
...
"We find that papers published by those authors from Chinese affiliations have much lower visibility on the social web than articles from other countries, when there is no significant difference for the citations. Fewer of China’s publications get tweeted, and those tweeted publications attract less social attention. A geographical analysis of tweeters shows that scholarly articles get most of their social attention from the authors’ home countries"

```{r echo = F, message = F, warning = F, fig.height = 24, fig.width = 6}

d <- read_csv("data/twitter_country_oa.csv") %>%
  filter(nchar(author_country) == 2,
         tweets != "NULL") %>%  na.omit() %>%
  mutate(tweets = as.numeric(tweets))

top_countries <- d %>%
  group_by(author_country) %>%
  summarize(tweets = sum(tweets)) %>%
  ungroup() %>%
  top_n(30, tweets) %>%
  pull(author_country)

countries_oa <- read_csv("data/altmetrics_coverage_country_oa.csv") %>%
  select(country, type, items)

oa_share <- read_csv("data/oa_share_year.csv") %>%
  summarize(closed = sum(closed),
            open = sum(open)) %>%
  pivot_longer(closed:open,
               names_to = "type",
               values_to = "items")

d %>%
  group_by(tweeter_country, type) %>%
  summarize(tweets = sum(tweets)) %>%
  ungroup() %>%
  inner_join(oa_share, by = "type") %>%
  group_by(tweeter_country) %>%
  mutate(tweets_per_item = tweets/items,
         rel_tweets_per_item = (tweets/items) / (sum(tweets)/sum(items))) %>%
  ungroup() %>%
  mutate(continent = countrycode::countrycode(tweeter_country, "iso2c", "continent")) %>%
  ggplot() +
  geom_point(aes(x = reorder(tweeter_country, desc(tweeter_country)), y = rel_tweets_per_item, fill = type),
             shape = 21) +
