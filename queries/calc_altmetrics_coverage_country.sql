...
SELECT
	...
	CAST(SUM(CASE WHEN t2.count_news = 0 THEN 0 ELSE 1 * t3.country_weight END) AS DECIMAL) AS news,
	CAST(SUM(CASE WHEN t2.count_policy = 0 THEN 0 ELSE 1 * t3.country_weight END) AS DECIMAL) AS policies,
	CAST(SUM(CASE WHEN t2.count_twitter = 0 THEN 0 ELSE 1 * t3.country_weight END) AS DECIMAL) AS twitter,
	CAST(SUM(CASE WHEN t2.count_wikipedia = 0 THEN 0 ELSE 1 * t3.country_weight END) AS DECIMAL) AS wikipedia
FROM
